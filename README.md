...
Если метод сервиса возвращает единичное значение (кроме ошибки), то оно автоматически будет помечено как inline. 
Данная аннотация отключает это поведение по умолчанию.
