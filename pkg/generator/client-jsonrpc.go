...
func (tr Transport) jsonrpcClientCallFunc(hasTrace bool) Code {
	...
	return Func().Params(Id("cli").Op("*").Id("ClientJsonRPC")).Id("jsonrpcCall").
		...
		bg.Line().List(Id("appID"), Id("_")).Op(":=").Id(_ctx_).Dot("Value").Call(Id("headerApplicationID")).Op(".(").String().Op(")")
		bg.If(Id("appID").Op("!=").Lit("")).Block(
			bg.Id("req").Dot("Header").Dot("Set").Call(Id("headerApplicationID"), Id("appID")),
		)
