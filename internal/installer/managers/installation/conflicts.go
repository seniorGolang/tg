	...
	for _, inst := range installations {
		...
		for _, file := range inst.Files {
			if existingPackageName, exists := destinationMap[file.Path]; exists {
				err = fmt.Errorf(i18n.Msg("File conflict: file %s is already installed by package %s, and package %s is also trying to install it"), file.Path, inst.Source+"/"+inst.Package, existingPackageName)
				return
			}
		}
	}
