...
func (m *manager) Uninstall(ctx context.Context, packageID string, keepFiles bool) (err error) {
	...
	if installation, err = m.databaseManager.GetInstallation(ctx, packageID); err != nil {
		err = fmt.Errorf(i18n.Msg("Installation not found: %w"), err)
		return
	}
	...
	if pkg != nil && pkg.Scripts != nil && pkg.Scripts.PreUninstall != nil {
		...
		if scopeConfig, err = storage.LoadScopeConfig(m.scopeName); err == nil {
			...
			if err = m.executeScript(ctx, pkg.Scripts.PreUninstall, workDir, nil); err != nil {
				err = fmt.Errorf(i18n.Msg("Error executing pre_uninstall script: %w"), err)
				return
			}
		}
	}
	...
	for _, file := range installation.Files {
		...
		if _, statErr = os.Stat(file.Path); statErr == nil {
			...
			if removeErr = os.Remove(file.Path); removeErr != nil {
				err = fmt.Errorf(i18n.Msg("Failed to remove file %s: %w"), file.Path, removeErr)
				return
			}
	...
	if err = m.databaseManager.RemoveInstallation(ctx, packageID); err != nil {
		err = fmt.Errorf(i18n.Msg("Failed to remove installation from database: %w"), err)
		return
	}
	...
	if pkg != nil && pkg.Scripts != nil && pkg.Scripts.PostUninstall != nil {
		...
		if scopeConfig, err = storage.LoadScopeConfig(m.scopeName); err == nil {
			...
			if err = m.executeScript(ctx, pkg.Scripts.PostUninstall, workDir, nil); err != nil {
				err = fmt.Errorf(i18n.Msg("Error executing post_uninstall script: %w"), err)
				return
			}
		}
	}
...
// Upgrade обновляет пакет.
func (m *manager) Upgrade(ctx context.Context, packageID string, newVersion models.Version) (err error) {

	err = errors.New(i18n.Msg("Package upgrade not implemented"))
	return
}

// Rollback откатывает установку.
func (m *manager) Rollback(ctx context.Context, installationID string) (err error) {

	err = errors.New(i18n.Msg("Installation rollback not implemented"))
	return
}
