...
func ParseDependency(dep string) (name string, version string, err error) {
	...
	return depModel.Package, depModel.Version, nil
}

func normalizeVersion(raw string) (normalized string) {

	trimmed := strings.TrimSpace(raw)
	if trimmed == "" {
		return ""
	}
	if strings.HasPrefix(trimmed, versionPrefixV) {
		return trimmed
	}
	return versionPrefixV + trimmed
}
...
func parseMajorVersion(major string) (majorInt int) {
	...
	raw := strings.TrimPrefix(major, versionPrefixV)
	if _, err := fmt.Sscanf(raw, "%d", &majorInt); err != nil {
		return 0
	}
	return
}
...
func incrementMinorVersion(majorMinor string) (nextMinor string) {
	...
	return majorPart + "." + fmt.Sprintf("%d", minorInt+1) + ".0"
}
