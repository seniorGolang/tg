// Copyright (c) 2025 Khramtsov Aleksei (seniorGolang@gmail.com).
// This file is subject to the terms and conditions defined in file 'LICENSE', which is part of this project source code.
package memory

import (
	"context"
	"errors"
	"fmt"

	"github.com/seniorGolang/tg/v3/internal/i18n"
)

func Allocate(ctx context.Context, h Host, size uint64) (ptr uint32, err error) {

	if h.GetMalloc() == nil {
		return 0, errors.New(i18n.Msg("malloc function is not available"))
	}

	if size == 0 {
		return 0, nil
	}

	var results []uint64
	if results, err = h.GetMalloc().Call(ctx, size); err != nil {
		return 0, fmt.Errorf(i18n.Msg("failed to allocate memory: %w"), err)
	}

	if len(results) == 0 {
		return 0, errors.New(i18n.Msg("malloc returned no results"))
	}

	allocatedPtr := results[0]
	if allocatedPtr > uint64(^uint32(0)) {
		return 0, errors.New(i18n.Msg("allocated pointer too large for uint32"))
	}

	return uint32(allocatedPtr), nil
}

func Free(ctx context.Context, h Host, ptr uint64) {

	if h.GetFree() == nil {
		return
	}

	if ptr == 0 {
		return
	}

	_, _ = h.GetFree().Call(ctx, ptr)
}
