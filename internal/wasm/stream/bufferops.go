...
func writeToRingBufferWithRetry(ctx context.Context, host *host.Host, bufferPtr uint32, dataSize uint32, data []byte, readReady chan struct{}) (written int, err error) {

	written = 0
	for written < len(data) {
		var w int
		if w, err = WriteToRingBuffer(ctx, host, bufferPtr, dataSize, data[written:]); err != nil {
			return written, err
		}
...
func readFromRingBufferWithRetry(ctx context.Context, host *host.Host, bufferPtr uint32, dataSize uint32, buffer []byte, writeReady chan struct{}) (n int, err error) {

	for {
		if n, err = ReadFromRingBuffer(ctx, host, bufferPtr, dataSize, buffer); err != nil {
			return n, err
		}
