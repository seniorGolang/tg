...
func New(ctx context.Context, wasmBytes []byte, info plugin.Info, rootDir string, logger plugin.Logger, opts ...Option) (h *host.Host, err error) {
	...
	// При -buildmode=c-shared точка входа — _initialize, не _start.
	fsBuilder := fs.NewBuilder(rootDir, hostOpts.TGPath)
	var fsConfig wazero.FSConfig
	var fsErr error
	fsConfig, fsErr = fsBuilder.Build(info)
	if fsErr != nil {
...
// getPluginDataDir: формат ~/.config/tg/data/{pluginName} (или $XDG_CONFIG_HOME). Директория создается при необходимости.
//
//nolint:unused // используется в newHost
func getPluginDataDir(pluginName string) (dataDir string, err error) {

	xdgConfigHome := os.Getenv("XDG_CONFIG_HOME")
	if xdgConfigHome != "" {
		dataDir = filepath.Join(xdgConfigHome, "tg", "data", pluginName)
	} else {
		homeDir, homeErr := os.UserHomeDir()
		if homeErr != nil {
			return "", homeErr
		}
		dataDir = filepath.Join(homeDir, ".config", "tg", "data", pluginName)
	}
...
// logPluginPermissions логирует информацию о правах плагина в режиме debug.
func logPluginPermissions(info plugin.Info, rootDir string, tgPath string) {

	if len(info.AllowedPaths) > 0 {
		mountPoints := fs.GetMountPoints(rootDir, tgPath, info)
		pathsList := make([]string, 0, len(mountPoints))
		for _, mp := range mountPoints {
			pathsList = append(pathsList, fmt.Sprintf("%s (%s) -> %s", mp.PathKey, mp.AccessLevel, mp.MountPoint))
		}
		slog.Debug(i18n.Msg("Plugin filesystem permissions"),
			"plugin", info.Name,
			"paths", pathsList)
	} else {
		slog.Debug(i18n.Msg("Plugin filesystem permissions"),
	...
	if len(info.AllowedEnvVars) > 0 {
		...
		slog.Debug(i18n.Msg("Plugin environment variables"),
			"plugin", info.Name,
			"variables", envKeys)
	} else {
		slog.Debug(i18n.Msg("Plugin environment variables"),
	...
	if len(info.Commands) > 0 {
		...
		slog.Debug(i18n.Msg("Plugin commands"),
			"plugin", info.Name,
