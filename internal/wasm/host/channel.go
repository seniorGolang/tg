...
func (cc *CallChannel) processCall(ctx context.Context, h *Host, req *CallRequest) (result CallResult) {
	...
	if funcPtr == nil {
		return CallResult{
			Error: fmt.Errorf("function %s not found in WASM module", req.FunctionName),
		}
	}
	...
	if len(req.Data) > 0 {
		if requestPtr, err = allocateMemory(ctx, h, uint64(len(req.Data))); err != nil {
			return CallResult{
				Error: fmt.Errorf("failed to allocate memory for request: %w", err),
			}
		}
		...
		if err = writeMemory(h, requestPtr, req.Data); err != nil {
			...
			return CallResult{
				Error: fmt.Errorf("failed to write request data to memory: %w", err),
			}
		}
	}
	...
	if err != nil {
		return CallResult{
			Error: fmt.Errorf("failed to call function %s: %w", req.FunctionName, err),
		}
	}
...
func (cc *CallChannel) Call(functionName string, data []byte) (resultChan <-chan CallResult) {
	...
	case <-cc.ctx.Done():
		ch <- CallResult{
			Error: errors.New("call channel is closed"),
		}
	default:
		ch <- CallResult{
			Error: errors.New("call channel is full: too many pending calls"),
		}
	}
