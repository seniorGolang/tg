...
// getDefaultTGPath: не использовать напрямую — @tg/ должен резолвиться
// в ConfigDir текущего scope. Если tgPath не передан явно, возвращается пустая строка.
func getDefaultTGPath() (path string) {

	// @tg/ должен резолвиться в ConfigDir текущего scope, который должен быть
	// передан явно через wasm.WithTGPath() или как параметр tgPath в fs.NewBuilder().
	// Если tgPath не передан, возвращаем пустую строку - это означает, что @tg/ не будет работать.
	return ""
}
