...
func HandleCompletionBash(ctx types.CommandContext) (err error) {
	...
	err = globalCompletionGenerator.GenBashCompletionV2(&buf, true)
	if err != nil {
...
func HandleCompletionZsh(ctx types.CommandContext) (err error) {
	...
	var buf bytes.Buffer
	err = globalCompletionGenerator.GenZshCompletion(&buf)
	if err != nil {
...
func HandleCompletionFish(ctx types.CommandContext) (err error) {
	...
	var buf bytes.Buffer
	err = globalCompletionGenerator.GenFishCompletion(&buf, true)
	if err != nil {
...
func HandleCompletionPowershell(ctx types.CommandContext) (err error) {
	...
	var buf bytes.Buffer
	err = globalCompletionGenerator.GenPowerShellCompletion(&buf)
	if err != nil {
...
func HandleCompletionInstall(ctx types.CommandContext) (err error) {
	...
	switch shell {
	case shellBash:
		err = globalCompletionGenerator.GenBashCompletionV2(&scriptBuf, true)
		if err != nil {
			errorMsg := fmt.Sprintf(i18n.Msg("Error generating %s completion: "), "bash") + err.Error()
			return errors.New(errorMsg)
		}
	...
	case shellZsh:
		err = globalCompletionGenerator.GenZshCompletion(&scriptBuf)
		if err != nil {
			errorMsg := fmt.Sprintf(i18n.Msg("Error generating %s completion: "), "zsh") + err.Error()
			return errors.New(errorMsg)
		}
	...
	case shellFish:
		err = globalCompletionGenerator.GenFishCompletion(&scriptBuf, true)
		if err != nil {
			errorMsg := fmt.Sprintf(i18n.Msg("Error generating %s completion: "), "fish") + err.Error()
			return errors.New(errorMsg)
		}
		fishCompletionsDir := filepath.Join(os.Getenv(homeEnvVar), fishCompletionsDir)
		completionFile = filepath.Join(fishCompletionsDir, cmdName+fishCompletionSuffix)
		sourceLine = ""
	default:
	...
	err = os.MkdirAll(completionDir, filePermDir)
	if err != nil {
		errorMsg := i18n.Msg("Error creating directory ") + completionDir + errorSeparator + err.Error()
		return errors.New(errorMsg)
	}

	err = os.WriteFile(completionFile, scriptBuf.Bytes(), filePermFile)
	if err != nil {
	...
	if configFile != "" {
		...
		if err != nil && !os.IsNotExist(err) {
			...
			if strings.Contains(string(configContent), completionFile) {
				...
				var file *os.File
				file, err = os.OpenFile(configFile, os.O_APPEND|os.O_CREATE|os.O_WRONLY, filePermFile)
				if err != nil {
