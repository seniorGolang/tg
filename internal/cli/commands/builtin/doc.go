...
func HandleDoc(ctx types.CommandContext) (err error) {
	...
	if pluginNameArg == "" {
		selectedItem, pluginName, version, err = handleInteractiveMode(ctx)
		if err != nil {
			return
		}
		if selectedItem == nil {
		...
		selectedItem, pluginName, version, err = handleDirectMode(ctx, pluginNameArg)
		if err != nil {
			return
		}
	}
...
func handleDirectMode(ctx types.CommandContext, pluginNameArg string) (selectedItem *docItem, pluginName string, version string, err error) {
	...
	if foundInstallation == nil {
		selectedItem, err = handleNotInstalled(ctx, inst, pluginName, version, scope)
		if err != nil {
			return
		}
	} else {
		selectedItem, err = handleInstalled(ctx, inst, pluginName, version, foundInstallation, scope)
		if err != nil {
			return
		}
	}
