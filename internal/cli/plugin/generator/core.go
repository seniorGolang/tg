...
func (c *CoreCreator) Create(rootDir string, moduleName string) (err error) {
	...
	for filename, templatePath := range wasmFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "wasm/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range execFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "exec/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range httpFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "http/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range dataFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "data/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range pluginFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "plugin/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range netFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "net/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range i18nFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "i18n/"+filename, err)
			return
		}
	}
	...
	for filename, templatePath := range manifestFiles {
		...
		if err = writeFile(filePath, content); err != nil {
			err = fmt.Errorf(i18n.Msg("Failed to create %s: %w"), "manifest/"+filename, err)
			return
		}
	}
